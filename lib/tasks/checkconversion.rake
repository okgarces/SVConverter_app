...
while 0==0 do
	...
	puts @parts
		...
		if @parts[0].to_i == STATE_PROCESSING
		    ...
		    puts exec "#{Rails.root}/bin/ffmpeg -y -i "+ video_url + " -vcodec libx264 -strict experimental #{Rails.root}/public/system/temp/"+ video_filename[0,video_filename.size-4]+".mp4"
			...
			video_converted_url = "#{Rails.root}/public/system/temp/"+ video_filename[0,video_filename.size-4]+".mp4"
			converted = File.open(video_converted_url)
